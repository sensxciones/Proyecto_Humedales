...
# se importa el modelo de YOLO, con algunos de los pesos.
...
while True:
    ...
    results = model(img,classes=14, stream=True) #al igual q el jupyter
    #al imponer classes = 14, solo detecta los objetos tipo "ave"
    # aqui se pasa por cada "ave" detectada
    for bird in results:
        boxes = bird.boxes
        for box in boxes:
            n = n+1
